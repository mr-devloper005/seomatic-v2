...
function normalizeIncomingContent(raw: string) {
  ...
  const hasHtmlTags = /<\/?[a-z][\s\S]*>/i.test(s);
  const escapeHtml = (t: string) => t.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
  ...
  if (!hasHtmlTags) {
    s = s
      ...
      .join("\n");
    if (!s.startsWith("<h") && !s.startsWith("<p")) s = `<p>${s}</p>`;
  ...
  s = s.replace(/\n{2,}/g, "</p><p>");
  s = s.replace(/<\/p>\s*<p>/g, "</p><p>");
  if (!/^<\w+/i.test(s.trim())) s = `<p>${s.replace(/\n/g, "<br/>")}</p>`;
...
export default function ContentEditorPage(): React.ReactElement {
  ...
  useEffect(() => {
    // drag & drop image support
    const el = (quillRef.current as any)?.editor?.root ?? document.querySelector(".ql-editor");
    if (!el) return;
    const onDrop = async (e: DragEvent) => {
      e.preventDefault();
      if (!e.dataTransfer) return;
