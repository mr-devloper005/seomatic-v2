...
// // src/components/ContentDataTable.tsx
...
//   const [storedItems, setStoredItems] = useLocalStorage<ContentItem[]>(
//     "content-items",
//     []
//   );
...
//       header: "Keyword(s)",
//       cell: ({ row }) => {
//         const list = row.original.keywordsUsed?.length
//           ? row.original.keywordsUsed
...
//       header: "Preview",
//       cell: ({ row }) => (
//         <div className="max-w-[320px] text-muted-foreground">
//           {makeSnippet(row.original.generatedContent, 50)}
//         </div>
//       ),
//     },
//     {
...
//         >
//           <IconFileSpreadsheet className="h-3 w-3" />
...
//     try {
//       const worksheetData = contentItems.map((item) => ({
//         "Keyword(s)": (item.keywordsUsed?.length
//           ? item.keywordsUsed
//           : [item.keyword]
//         ).join(", "),
//         Title: item.title ?? "",
...
//         "Generated Content"
//       );
//       const fileName = `all-generated-content-${
...
//           <CardDescription>
//             {projectId
//               ? "No content for this file yet."
//               : "Content generated from your uploaded Excel files will appear here."}
//           </CardDescription>
//         </CardHeader>
//         <CardContent>
//           <div className="text-center py-8">
...
//             <p className="text-muted-foreground">
//               {projectId
//                 ? "Select another file or wait for processing to complete."
...
//               className="flex items-center gap-2"
//             >
//               <IconDownload className="h-4 w-4" /> Download All
//             </Button>
//           </div>
//         </div>
//       </CardHeader>
...
// // components/content-data-table.tsx
...
// interface Props {
//   projectId?: string;
//   /** Optional: parent se aaya hua state (useContentGeneration) */
//   contentItems?: ContentItem[] | null;
//   /** Optional: parent updater so UI instantly syncs */
//   onItemsChange?: (updater: (prev: ContentItem[]) => ContentItem[]) => void;
// }
...
//   projectId,
//   contentItems: propContentItems,
//   onItemsChange,
// }: Props) {
//   const [sorting, setSorting] = useState<SortingState>([]);
//   const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);

//   // Local fallback (agar parent se items na aaye)
//   const [storedItems, setStoredItems] = useLocalStorage<ContentItem[]>(
//     "content-items",
//     []
//   );

//   const useExternal = !!propContentItems && !!onItemsChange;

//   const allItems: ContentItem[] = useExternal
//     ? (propContentItems as ContentItem[])
//     : storedItems || [];
...
//     if (useExternal) {
//       // Parent state + localStorage dono update honge via hook
//       onItemsChange?.((prev = []) =>
//         (prev || []).filter((i) => i.id !== id)
//       );
//     } else {
//       // Pure local-table mode
//       setStoredItems((prev = []) =>
//         (prev || []).filter((i) => i.id !== id)
//       );
//     }
//     toast.success("Content row deleted");
//   };
...
//       cell: ({ row }) => (
//         <div className="max-w-[320px] text-muted-foreground">
//           {makeSnippet(row.original.generatedContent, 60)}
//         </div>
//       ),
//     },
//     {
...
//             variant="outline"
//             size="sm"
//             className="flex items-center gap-1 text-rose-600 border-rose-300 hover:bg-rose-50"
...
//     try {
//       const worksheetData = visibleItems.map((item, index) => ({
//         "#": index + 1,
//         "Keyword(s)": (item.keywordsUsed?.length
//           ? item.keywordsUsed
//           : [item.keyword]
...
//         "Generated Content"
//       );
//       const fileName = `generated-content-${
//         new Date().toISOString().split("T")[0]
//       }.xlsx`;
//       XLSX.writeFile(workbook, fileName);
//       toast.success(`Downloaded ${visibleItems.length} item(s)`);
//     } catch (err) {
//       console.error(err);
//       toast.error("Failed to download file");
//     }
...
interface Props {
  projectId?: string;
  contentItems?: ContentItem[] | null; // parent se aaya hua list
  onItemsChange?: (updater: (prev: ContentItem[]) => ContentItem[]) => void; // parent updater
}
...
export function ContentDataTable({
  ...
  // Fallback: jab parent se data na mile (old usage)
  const [storedItems, setStoredItems] = useLocalStorage<ContentItem[]>(
    "content-items",
    []
  );

  // 👉 Agar parent ne contentItems diya hai, wahi use hoga (live)
  const useExternal = Array.isArray(propContentItems);
  ...
  const handleDeleteRow = (id: string) => {
    if (useExternal && onItemsChange) {
      onItemsChange((prev = []) => (prev || []).filter((i) => i.id !== id));
    } else if (!useExternal) {
      setStoredItems((prev = []) => (prev || []).filter((i) => i.id !== id));
    }
    toast.success("Content row deleted");
  };
  ...
  const columns: ColumnDef<ContentItem>[] = [
    {
      ...
      header: "#",
      cell: ({ row }) => (
        <div className="w-6 text-xs text-muted-foreground">{row.index + 1}</div>
      ),
      size: 24,
    },
    {
